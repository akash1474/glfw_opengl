...
function Bool(value)   return ffi.new("bool", value or false) end
function Int(value)    return ffi.new("int", value or 0) end
function UInt(value)   return ffi.new("unsigned int", value or 0) end
function Float(value)  return ffi.new("float", value or 0.0) end
function Double(value) return ffi.new("double", value or 0.0) end
function Char(value)   return ffi.new("char", value or 0) end

function BoolPtr(value)
    local ptr = ffi.new("bool[1]")
    ptr[0] = value or false
    return ptr
end

function IntPtr(value)
    local ptr = ffi.new("int[1]")
    ptr[0] = value or 0
    return ptr
end

function UIntPtr(value)
    local ptr = ffi.new("unsigned int[1]")
    ptr[0] = value or 0
    return ptr
end

function FloatPtr(value)
    local ptr = ffi.new("float[1]")
    ptr[0] = value or 0.0
    return ptr
end

function DoublePtr(value)
    local ptr = ffi.new("double[1]")
    ptr[0] = value or 0.0
    return ptr
end

function CharPtr(value)
...
function Float2(x, y)
    return ffi.new("float[2]", {x or 0.0, y or 0.0})
end

function Float3(x, y, z)
    return ffi.new("float[3]", {x or 0.0, y or 0.0, z or 0.0})
end

function Float4(x, y, z, w)
    return ffi.new("float[4]", {x or 0.0, y or 0.0, z or 0.0, w or 1.0})
end


function FromPtr(ptr)
    if ptr == nil then error("FromPtr() - got nil pointer") end

    local ctype = tostring(ffi.typeof(ptr))
    local scalar_types = {
        ["ctype<bool [1]>"] = function(p) return p[0] end,
        ["ctype<int [1]>"] = function(p) return p[0] end,
        ["ctype<unsigned int [1]>"] = function(p) return tonumber(p[0]) end,
        ["ctype<float [1]>"] = function(p) return tonumber(p[0]) end,
        ["ctype<double [1]>"] = function(p) return tonumber(p[0]) end,
        ["ctype<char [1]>"] = function(p) return string.char(p[0]) end,
    }

    local fn = scalar_types[ctype]
    if not fn then
        error("FromPtr() - Unsupported pointer type: " .. ctype)
    end
    return fn(ptr)
end
...
-- Pointer Converter
function ToPtr(val)
    local ctype = tostring(ffi.typeof(val))
    local type_map = {
        ["ctype<bool>"] = "bool[1]",
        ["ctype<int>"] = "int[1]",
        ["ctype<unsigned int>"] = "unsigned int[1]",
        ["ctype<float>"] = "float[1]",
        ["ctype<double>"] = "double[1]",
        ["ctype<char>"] = "char[1]",
    }

    local ptr_type = type_map[ctype]
    if not ptr_type then
        error("ToPtr() - Unsupported type: " .. ctype)
    end

    return ffi.new(ptr_type, val)
end

-- CString Helper
--[[
    Creates a mutable C string buffer wrapped in a Lua object.
    This is essential for ImGui functions like InputText that need to modify
    a character buffer.

    Features:
    - Safe buffer modification from Lua.
    - Easy printing for debugging via `print()`.
    - Clean API to get the raw pointer or the Lua string.

    API:
    - `cdata()`: Returns the raw `char*` for passing to C functions.
    - `get()`:   Returns the current content as a Lua string.
    - `set(str)`: Safely updates the buffer's content.
]]
function CString(str, max_len)
    max_len = max_len or (#str + 1)
    local buf = ffi.new("char[?]", max_len)
    ffi.copy(buf, str)

    local wrapper = { _buf = buf, len = max_len }

    setmetatable(wrapper, {
        __tostring = function(self)
            return ffi.string(self._buf)
        end,
        __index = {
            -- Explicitly expose the raw cdata pointer via a function
            cdata = function(self) return self._buf end,
            get = function(self) return ffi.string(self._buf) end,
            set = function(self, new_str)
                -- Add a safety check for buffer overflow
                ffi.copy(self._buf, new_str, math.min(#new_str, self.len - 1))
            end,
        }
    })

    return wrapper, max_len
end


-- ImVec2 Helper
--[[
    Creates a 2D vector object for positions, sizes, etc.
    It wraps a C `ImVec2` pointer to be compatible with ImGui

    Features:
    - Direct component access (e.g., `myVec.x`).
    - Operator overloading for vector math (`+`, `-`, `*`).
    - Easy printing for debugging via `print()`.

    API:
    - `cdata()`:   Returns the raw `ImVec2*` for passing to C functions.
    - `set(x, y)`: Updates the vector's components.
    - `unpack()`:  Returns the components as multiple Lua numbers.
]]
local ImVec2_methods = {
    cdata = function(self) return self._cdata end,
    set = function(self, x, y) self._cdata[0].x = x; self._cdata[0].y = y end,
    unpack = function(self) return self._cdata[0].x, self._cdata[0].y end
}

local ImVec2_meta = {
    __index = function(self, key)
        return ImVec2_methods[key] or self._cdata[0][key]
    end,

    __newindex = function(self, key, value)
        self._cdata[0][key] = value
    end,
    __tostring = function(self)
        return string.format("ImVec2(%.2f, %.2f)", self.x, self.y)
    end,
    __add = function(lhs, rhs) return ImVec2(lhs.x + rhs.x, lhs.y + rhs.y) end,
    __sub = function(lhs, rhs) return ImVec2(lhs.x - rhs.x, lhs.y - rhs.y) end,
    __unm = function(vec) return ImVec2(-vec.x, -vec.y) end,
...
function ImVec2(x, y)
    local cdata_ptr = ffi.new("ImVec2_C[1]")
    cdata_ptr[0].x = x or 0
    cdata_ptr[0].y = y or 0
    local wrapper = { _cdata = cdata_ptr }
    return setmetatable(wrapper, ImVec2_meta)
end

-- ImVec4 Helper
--[[
    Creates a 4D vector object, often used for colors (R,G,B,A). Wraps a C `ImVec4` pointer.

    Features:
    - Direct component access (e.g., `myColor.w`).
    - Operator overloading for vector math (`+`, `-`, `*`).
    - Easy printing for debugging via `print()`.

    API:
    - `cdata()`:      Returns the raw `ImVec4*` for passing to C functions.
    - `set(x,y,z,w)`: Updates the vector's components.
    - `unpack()`:     Returns the components as multiple Lua numbers.
]]
local ImVec4_methods = {
    cdata = function(self) return self._cdata end,
    set = function(self, x, y, z, w) self._cdata[0].x = x; self._cdata[0].y = y; self._cdata[0].z = z; self._cdata[0].w = w end,
    unpack = function(self) return self._cdata[0].x, self._cdata[0].y, self._cdata[0].z, self._cdata[0].w end
}

local ImVec4_meta = {
    __index = function(self, key)
        return ImVec4_methods[key] or self._cdata[0][key]
    end,
    __newindex = function(self, key, value)
        self._cdata[0][key] = value
    end,
    __tostring = function(self)
        return string.format("ImVec4(%.2f, %.2f, %.2f, %.2f)", self.x, self.y, self.z, self.w)
    end,
    __add = function(lhs, rhs) return ImVec4(lhs.x + rhs.x, lhs.y + rhs.y, lhs.z + rhs.z, lhs.w + rhs.w) end,
    __sub = function(lhs, rhs) return ImVec4(lhs.x - rhs.x, lhs.y - rhs.y, lhs.z - rhs.z, lhs.w - rhs.w) end,
    __unm = function(vec) return ImVec4(-vec.x, -vec.y, -vec.z, -vec.w) end,
    __mul = function(lhs, rhs)
        if type(rhs) == "number" then -- vector * scalar
            return ImVec4(lhs.x * rhs, lhs.y * rhs, lhs.z * rhs, lhs.w * rhs)
        elseif type(lhs) == "number" then -- scalar * vector
            return ImVec4(rhs.x * lhs, rhs.y * lhs, rhs.z * lhs, rhs.w * lhs)
...
function ImVec4(x, y, z, w)
    local cdata_ptr = ffi.new("ImVec4_C[1]")
    cdata_ptr[0].x = x or 0
    cdata_ptr[0].y = y or 0
    cdata_ptr[0].z = z or 0
    cdata_ptr[0].w = w or 0
    local wrapper = { _cdata = cdata_ptr }
    return setmetatable(wrapper, ImVec4_meta)
end
...
--===
--=== struct ImTextureRef ===
function M.ImTextureRef() return ffi.new("ImTextureRef") end
function M.ImTextureRefPtr() return ffi.new("ImTextureRef[1]") end
function M.ImTextureRef(tex_id)
  local res = ffi.new("ImTextureRef")
  res.tex_id = tex_id
  return res
end
function M.ImTextureRefPtr(tex_id)end
--===
...
function M.ShowIDStackToolWindow(bool_p_open) 
  -- bool_p_open is optional and can be nil
  C.imgui_ShowIDStackToolWindow(bool_p_open)
end
...
function M.BeginChild1(string_str_id, ImVec2_size, ImGuiChildFlags_child_flags, ImGuiWindowFlags_window_flags) 
  if ImVec2_size == nil then ImVec2_size = ffi.new("ImVec2_C",  0, 0) end
  if ImGuiChildFlags_child_flags == nil then ImGuiChildFlags_child_flags = 0 end
  if ImGuiWindowFlags_window_flags == nil then ImGuiWindowFlags_window_flags = 0 end
  ...
  return C.imgui_BeginChild1(string_str_id, ImVec2_size, ImGuiChildFlags_child_flags, ImGuiWindowFlags_window_flags)
end
function M.BeginChild2(ImGuiID_id, ImVec2_size, ImGuiChildFlags_child_flags, ImGuiWindowFlags_window_flags) 
  if ImVec2_size == nil then ImVec2_size = ffi.new("ImVec2_C",  0, 0) end
  if ImGuiChildFlags_child_flags == nil then ImGuiChildFlags_child_flags = 0 end
  if ImGuiWindowFlags_window_flags == nil then ImGuiWindowFlags_window_flags = 0 end
  return C.imgui_BeginChild2(ImGuiID_id, ImVec2_size, ImGuiChildFlags_child_flags, ImGuiWindowFlags_window_flags)
end
...
function M.SetNextWindowPos(ImVec2_pos, ImGuiCond_cond, ImVec2_pivot) 
  ...
  if ImVec2_pivot == nil then ImVec2_pivot = ffi.new("ImVec2_C",  0, 0) end
...
function M.PushFont(ImFont_font, float_font_size_base_unscaled) 
  ...
  C.imgui_PushFont(ImFont_font, float_font_size_base_unscaled)
end
...
function M.GetFontBaked() return C.imgui_GetFontBaked() end
...
function M.PushStyleVarX(ImGuiStyleVar_idx, float_val_x) C.imgui_PushStyleVarX(ImGuiStyleVar_idx, float_val_x) end
function M.PushStyleVarY(ImGuiStyleVar_idx, float_val_y) C.imgui_PushStyleVarY(ImGuiStyleVar_idx, float_val_y) end
function M.PopStyleVar(int_count) 
  if int_count == nil then int_count = 1 end
  C.imgui_PopStyleVar(int_count)
end
function M.PushItemFlag(ImGuiItemFlags_option, bool_enabled) C.imgui_PushItemFlag(ImGuiItemFlags_option, bool_enabled) end
function M.PopItemFlag() C.imgui_PopItemFlag() end
...
function M.PushTextWrapPos(float_wrap_local_pos_x) 
  if float_wrap_local_pos_x == nil then float_wrap_local_pos_x = 0.0 end
...
function M.GetColorU321(ImGuiCol_idx, float_alpha_mul) 
  if float_alpha_mul == nil then float_alpha_mul = 1.0 end
...
function M.GetColorU323(ImU32_col, float_alpha_mul) 
  if float_alpha_mul == nil then float_alpha_mul = 1.0 end
  return C.imgui_GetColorU323(ImU32_col, float_alpha_mul)
end
...
function M.SameLine(float_offset_from_start_x, float_spacing) 
  if float_offset_from_start_x == nil then float_offset_from_start_x = 0.0 end
  if float_spacing == nil then float_spacing = - 1.0 end
...
function M.Indent(float_indent_w) 
  if float_indent_w == nil then float_indent_w = 0.0 end
...
function M.GetID4(int_int_id) return C.imgui_GetID4(int_int_id) end
...
function M.ProgressBar(float_fraction, ImVec2_size_arg, string_overlay) 
  if ImVec2_size_arg == nil then ImVec2_size_arg = ffi.new("ImVec2_C",  - FLT_MIN, 0) end
...
function M.TextLink(string_label) 
  if string_label == nil then log("E", "", "Parameter 'string_label' of function 'TextLink' cannot be nil, as the c type is 'const char *'") ; return end
  return C.imgui_TextLink(string_label)
end
function M.TextLinkOpenURL(string_label, string_url) 
  -- string_url is optional and can be nil
  if string_label == nil then log("E", "", "Parameter 'string_label' of function 'TextLinkOpenURL' cannot be nil, as the c type is 'const char *'") ; return end
  return C.imgui_TextLinkOpenURL(string_label, string_url)
end
function M.Image(ImTextureID_tex_ref, ImVec2_image_size, ImVec2_uv0, ImVec2_uv1) 
  if ImVec2_uv0 == nil then ImVec2_uv0 = ffi.new("ImVec2_C",  0, 0) end
  if ImVec2_uv1 == nil then ImVec2_uv1 = ffi.new("ImVec2_C",  1, 1) end
  C.imgui_Image(ImTextureID_tex_ref, ImVec2_image_size, ImVec2_uv0, ImVec2_uv1)
end
function M.ImageWithBg(ImTextureID_tex_ref, ImVec2_image_size, ImVec2_uv0, ImVec2_uv1, ImVec4_bg_col, ImVec4_tint_col) 
  if ImVec2_uv0 == nil then ImVec2_uv0 = ffi.new("ImVec2_C",  0, 0) end
  if ImVec2_uv1 == nil then ImVec2_uv1 = ffi.new("ImVec2_C",  1, 1) end
  if ImVec4_bg_col == nil then ImVec4_bg_col = ffi.new("ImVec4_C",  0, 0, 0, 0) end
  if ImVec4_tint_col == nil then ImVec4_tint_col = ffi.new("ImVec4_C",  1, 1, 1, 1) end
  C.imgui_ImageWithBg(ImTextureID_tex_ref, ImVec2_image_size, ImVec2_uv0, ImVec2_uv1, ImVec4_bg_col, ImVec4_tint_col)
end
function M.ImageButton(string_str_id, ImTextureID_tex_ref, ImVec2_image_size, ImVec2_uv0, ImVec2_uv1, ImVec4_bg_col, ImVec4_tint_col) 
  if ImVec2_uv0 == nil then ImVec2_uv0 = ffi.new("ImVec2_C",  0, 0) end
  if ImVec2_uv1 == nil then ImVec2_uv1 = ffi.new("ImVec2_C",  1, 1) end
  if ImVec4_bg_col == nil then ImVec4_bg_col = ffi.new("ImVec4_C",  0, 0, 0, 0) end
  if ImVec4_tint_col == nil then ImVec4_tint_col = ffi.new("ImVec4_C",  1, 1, 1, 1) end
  ...
  return C.imgui_ImageButton(string_str_id, ImTextureID_tex_ref, ImVec2_image_size, ImVec2_uv0, ImVec2_uv1, ImVec4_bg_col, ImVec4_tint_col)
end
...
function M.Combo3(string_label, int_current_item, functionPtr_getter, void_user_data, int_items_count, int_popup_max_height_in_items) 
  if int_popup_max_height_in_items == nil then int_popup_max_height_in_items = - 1 end
  ...
  if functionPtr_getter == nil then log("E", "", "Parameter 'functionPtr_getter' of function 'Combo3' cannot be nil, as the c type is 'const char *(*)(void *, int)'") ; return end
  if void_user_data == nil then log("E", "", "Parameter 'void_user_data' of function 'Combo3' cannot be nil, as the c type is 'void *'") ; return end
  return C.imgui_Combo3(string_label, int_current_item, functionPtr_getter, void_user_data, int_items_count, int_popup_max_height_in_items)
end
function M.DragFloat(string_label, float_v, float_v_speed, float_v_min, float_v_max, string_format, ImGuiSliderFlags_flags) 
  if float_v_speed == nil then float_v_speed = 1.0 end
  if float_v_min == nil then float_v_min = 0.0 end
  if float_v_max == nil then float_v_max = 0.0 end
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'DragFloat' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.DragFloat2(string_label, floatPtr_v, float_v_speed, float_v_min, float_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'DragFloat2' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.DragFloat3(string_label, floatPtr_v, float_v_speed, float_v_min, float_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'DragFloat3' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.DragFloat4(string_label, floatPtr_v, float_v_speed, float_v_min, float_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'DragFloat4' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.DragFloatRange2(string_label, float_v_current_min, float_v_current_max, float_v_speed, float_v_min, float_v_max, string_format, string_format_max, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'DragFloatRange2' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.DragInt(string_label, int_v, float_v_speed, int_v_min, int_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'DragInt' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.DragInt2(string_label, intPtr_v, float_v_speed, int_v_min, int_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'DragInt2' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.DragInt3(string_label, intPtr_v, float_v_speed, int_v_min, int_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'DragInt3' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.DragInt4(string_label, intPtr_v, float_v_speed, int_v_min, int_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'DragInt4' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.DragIntRange2(string_label, int_v_current_min, int_v_current_max, float_v_speed, int_v_min, int_v_max, string_format, string_format_max, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'DragIntRange2' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.SliderFloat(string_label, float_v, float_v_min, float_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'SliderFloat' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.SliderFloat2(string_label, floatPtr_v, float_v_min, float_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'SliderFloat2' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.SliderFloat3(string_label, floatPtr_v, float_v_min, float_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'SliderFloat3' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.SliderFloat4(string_label, floatPtr_v, float_v_min, float_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'SliderFloat4' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.SliderAngle(string_label, float_v_rad, float_v_degrees_min, float_v_degrees_max, string_format, ImGuiSliderFlags_flags) 
  if float_v_degrees_min == nil then float_v_degrees_min = - 360.0 end
  if float_v_degrees_max == nil then float_v_degrees_max =  360.0 end
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'SliderAngle' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.SliderInt(string_label, int_v, int_v_min, int_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'SliderInt' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.SliderInt2(string_label, intPtr_v, int_v_min, int_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'SliderInt2' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.SliderInt3(string_label, intPtr_v, int_v_min, int_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'SliderInt3' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.SliderInt4(string_label, intPtr_v, int_v_min, int_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'SliderInt4' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.VSliderFloat(string_label, ImVec2_size, float_v, float_v_min, float_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'VSliderFloat' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.VSliderInt(string_label, ImVec2_size, int_v, int_v_min, int_v_max, string_format, ImGuiSliderFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'VSliderInt' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.InputFloat(string_label, float_v, float_step, float_step_fast, string_format, ImGuiInputTextFlags_flags) 
  if float_step == nil then float_step = 0.0 end
  if float_step_fast == nil then float_step_fast = 0.0 end
  if string_format == nil then string_format = "%.3f" end
  if ImGuiInputTextFlags_flags == nil then ImGuiInputTextFlags_flags = 0 end
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'InputFloat' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.InputFloat2(string_label, floatPtr_v, string_format, ImGuiInputTextFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'InputFloat2' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.InputFloat3(string_label, floatPtr_v, string_format, ImGuiInputTextFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'InputFloat3' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.InputFloat4(string_label, floatPtr_v, string_format, ImGuiInputTextFlags_flags) 
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'InputFloat4' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.InputDouble(string_label, double_v, double_step, double_step_fast, string_format, ImGuiInputTextFlags_flags) 
  if double_step == nil then double_step = 0.0 end
  if double_step_fast == nil then double_step_fast = 0.0 end
  ...
  if string_format == nil then log("E", "", "Parameter 'string_format' of function 'InputDouble' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.SetNextItemStorageID(ImGuiID_storage_id) C.imgui_SetNextItemStorageID(ImGuiID_storage_id) end
...
function M.BeginMultiSelect(ImGuiMultiSelectFlags_flags, int_selection_size, int_items_count) 
  if int_selection_size == nil then int_selection_size = - 1 end
  if int_items_count == nil then int_items_count = - 1 end
  return C.imgui_BeginMultiSelect(ImGuiMultiSelectFlags_flags, int_selection_size, int_items_count)
end
function M.EndMultiSelect() return C.imgui_EndMultiSelect() end
function M.SetNextItemSelectionUserData(ImGuiSelectionUserData_selection_user_data) C.imgui_SetNextItemSelectionUserData(ImGuiSelectionUserData_selection_user_data) end
function M.IsItemToggledSelection() return C.imgui_IsItemToggledSelection() end
...
function M.ListBox2(string_label, int_current_item, functionPtr_getter, void_user_data, int_items_count, int_height_in_items) 
  if int_height_in_items == nil then int_height_in_items = - 1 end
  ...
  if functionPtr_getter == nil then log("E", "", "Parameter 'functionPtr_getter' of function 'ListBox2' cannot be nil, as the c type is 'const char *(*)(void *, int)'") ; return end
  if void_user_data == nil then log("E", "", "Parameter 'void_user_data' of function 'ListBox2' cannot be nil, as the c type is 'void *'") ; return end
  return C.imgui_ListBox2(string_label, int_current_item, functionPtr_getter, void_user_data, int_items_count, int_height_in_items)
end
function M.PlotLines1(string_label, float_values, int_values_count, int_values_offset, string_overlay_text, float_scale_min, float_scale_max, ImVec2_graph_size, int_stride) 
  ...
  if float_scale_min == nil then float_scale_min = math.huge end
  if float_scale_max == nil then float_scale_max = math.huge end
  if ImVec2_graph_size == nil then ImVec2_graph_size = ffi.new("ImVec2_C",  0, 0) end
...
function M.BeginTable(string_str_id, int_columns, ImGuiTableFlags_flags, ImVec2_outer_size, float_inner_width) 
  if ImGuiTableFlags_flags == nil then ImGuiTableFlags_flags = 0 end
  if ImVec2_outer_size == nil then ImVec2_outer_size = ffi.new("ImVec2_C",  0.0, 0.0) end
  if float_inner_width == nil then float_inner_width = 0.0 end
  ...
  return C.imgui_BeginTable(string_str_id, int_columns, ImGuiTableFlags_flags, ImVec2_outer_size, float_inner_width)
end
...
function M.TableNextRow(ImGuiTableRowFlags_row_flags, float_min_row_height) 
  ...
  if float_min_row_height == nil then float_min_row_height = 0.0 end
...
function M.TableSetupColumn(string_label, ImGuiTableColumnFlags_flags, float_init_width_or_weight, ImGuiID_user_id) 
  ...
  if float_init_width_or_weight == nil then float_init_width_or_weight = 0.0 end
...
function M.TableAngledHeadersRow() C.imgui_TableAngledHeadersRow() end
...
function M.TableGetHoveredColumn() return C.imgui_TableGetHoveredColumn() end
...
function M.Columns(int_count, string_id, bool_borders) 
  if int_count == nil then int_count = 1 end
  -- string_id is optional and can be nil
  if bool_borders == nil then bool_borders = true end
  C.imgui_Columns(int_count, string_id, bool_borders)
end
...
function M.EndDisabled() C.imgui_EndDisabled() end
...
function M.SetNavCursorVisible(bool_visible) C.imgui_SetNavCursorVisible(bool_visible) end
function M.SetNextItemAllowOverlap() C.imgui_SetNextItemAllowOverlap() end
...
function M.GetItemID() return C.imgui_GetItemID() end
...
function M.GetBackgroundDrawList() return C.imgui_GetBackgroundDrawList() end
function M.GetForegroundDrawList() return C.imgui_GetForegroundDrawList() end
...
function M.CalcTextSize(string_text, string_text_end, bool_hide_text_after_double_hash, float_wrap_width) 
  ...
  if float_wrap_width == nil then float_wrap_width = - 1.0 end
...
function M.IsKeyReleased(ImGuiKey_key) return C.imgui_IsKeyReleased(ImGuiKey_key) end
function M.IsKeyChordPressed(ImGuiKeyChord_key_chord) return C.imgui_IsKeyChordPressed(ImGuiKeyChord_key_chord) end
function M.GetKeyPressedAmount(ImGuiKey_key, float_repeat_delay, float_rate) return C.imgui_GetKeyPressedAmount(ImGuiKey_key, float_repeat_delay, float_rate) end
...
function M.Shortcut(ImGuiKeyChord_key_chord, ImGuiInputFlags_flags) 
  if ImGuiInputFlags_flags == nil then ImGuiInputFlags_flags = 0 end
  return C.imgui_Shortcut(ImGuiKeyChord_key_chord, ImGuiInputFlags_flags)
end
function M.SetNextItemShortcut(ImGuiKeyChord_key_chord, ImGuiInputFlags_flags) 
  if ImGuiInputFlags_flags == nil then ImGuiInputFlags_flags = 0 end
  C.imgui_SetNextItemShortcut(ImGuiKeyChord_key_chord, ImGuiInputFlags_flags)
end
function M.SetItemKeyOwner(ImGuiKey_key) C.imgui_SetItemKeyOwner(ImGuiKey_key) end
...
function M.IsMouseReleasedWithDelay(ImGuiMouseButton_button, float_delay) return C.imgui_IsMouseReleasedWithDelay(ImGuiMouseButton_button, float_delay) end
...
function M.IsMouseDragging(ImGuiMouseButton_button, float_lock_threshold) 
  if float_lock_threshold == nil then float_lock_threshold = - 1.0 end
...
function M.DebugFlashStyleColor(ImGuiCol_idx) C.imgui_DebugFlashStyleColor(ImGuiCol_idx) end
function M.DebugStartItemPicker() C.imgui_DebugStartItemPicker() end
...
function M.DebugLog(string_fmt, ...) 
  if string_fmt == nil then log("E", "", "Parameter 'string_fmt' of function 'DebugLog' cannot be nil, as the c type is 'const char *'") ; return end
  C.imgui_DebugLog(string_fmt, ...)
end
function M.DebugLogV(string_fmt, va_list_args) 
  if string_fmt == nil then log("E", "", "Parameter 'string_fmt' of function 'DebugLogV' cannot be nil, as the c type is 'const char *'") ; return end
  C.imgui_DebugLogV(string_fmt, va_list_args)
end
...
M.ChildFlags_None = C.ImGuiChildFlags_None
M.ChildFlags_Borders = C.ImGuiChildFlags_Borders
M.ChildFlags_AlwaysUseWindowPadding = C.ImGuiChildFlags_AlwaysUseWindowPadding
M.ChildFlags_ResizeX = C.ImGuiChildFlags_ResizeX
M.ChildFlags_ResizeY = C.ImGuiChildFlags_ResizeY
M.ChildFlags_AutoResizeX = C.ImGuiChildFlags_AutoResizeX
M.ChildFlags_AutoResizeY = C.ImGuiChildFlags_AutoResizeY
M.ChildFlags_AlwaysAutoResize = C.ImGuiChildFlags_AlwaysAutoResize
M.ChildFlags_FrameStyle = C.ImGuiChildFlags_FrameStyle
M.ChildFlags_NavFlattened = C.ImGuiChildFlags_NavFlattened
--===
--=== enum ImGuiItemFlags_ ===
M.ItemFlags_None = C.ImGuiItemFlags_None
M.ItemFlags_NoTabStop = C.ImGuiItemFlags_NoTabStop
M.ItemFlags_NoNav = C.ImGuiItemFlags_NoNav
M.ItemFlags_NoNavDefaultFocus = C.ImGuiItemFlags_NoNavDefaultFocus
M.ItemFlags_ButtonRepeat = C.ImGuiItemFlags_ButtonRepeat
M.ItemFlags_AutoClosePopups = C.ImGuiItemFlags_AutoClosePopups
M.ItemFlags_AllowDuplicateId = C.ImGuiItemFlags_AllowDuplicateId
--===
...
M.InputTextFlags_EscapeClearsAll = C.ImGuiInputTextFlags_EscapeClearsAll
...
M.InputTextFlags_AlwaysOverwrite = C.ImGuiInputTextFlags_AlwaysOverwrite
M.InputTextFlags_AutoSelectAll = C.ImGuiInputTextFlags_AutoSelectAll
M.InputTextFlags_ParseEmptyRefVal = C.ImGuiInputTextFlags_ParseEmptyRefVal
M.InputTextFlags_DisplayEmptyRefVal = C.ImGuiInputTextFlags_DisplayEmptyRefVal
...
M.InputTextFlags_ElideLeft = C.ImGuiInputTextFlags_ElideLeft
...
M.InputTextFlags_WordWrap = C.ImGuiInputTextFlags_WordWrap
--===
...
M.TreeNodeFlags_AllowOverlap = C.ImGuiTreeNodeFlags_AllowOverlap
...
M.TreeNodeFlags_SpanLabelWidth = C.ImGuiTreeNodeFlags_SpanLabelWidth
M.TreeNodeFlags_SpanAllColumns = C.ImGuiTreeNodeFlags_SpanAllColumns
M.TreeNodeFlags_LabelSpanAllColumns = C.ImGuiTreeNodeFlags_LabelSpanAllColumns
M.TreeNodeFlags_NavLeftJumpsToParent = C.ImGuiTreeNodeFlags_NavLeftJumpsToParent
M.TreeNodeFlags_CollapsingHeader = C.ImGuiTreeNodeFlags_CollapsingHeader
M.TreeNodeFlags_DrawLinesNone = C.ImGuiTreeNodeFlags_DrawLinesNone
M.TreeNodeFlags_DrawLinesFull = C.ImGuiTreeNodeFlags_DrawLinesFull
M.TreeNodeFlags_DrawLinesToNodes = C.ImGuiTreeNodeFlags_DrawLinesToNodes
--===
...
M.PopupFlags_NoReopen = C.ImGuiPopupFlags_NoReopen
...
M.SelectableFlags_NoAutoClosePopups = C.ImGuiSelectableFlags_NoAutoClosePopups
...
M.SelectableFlags_AllowOverlap = C.ImGuiSelectableFlags_AllowOverlap
M.SelectableFlags_Highlight = C.ImGuiSelectableFlags_Highlight
M.SelectableFlags_SelectOnNav = C.ImGuiSelectableFlags_SelectOnNav
--===
...
M.ComboFlags_WidthFitPreview = C.ImGuiComboFlags_WidthFitPreview
...
M.TabBarFlags_DrawSelectedOverline = C.ImGuiTabBarFlags_DrawSelectedOverline
M.TabBarFlags_FittingPolicyMixed = C.ImGuiTabBarFlags_FittingPolicyMixed
M.TabBarFlags_FittingPolicyShrink = C.ImGuiTabBarFlags_FittingPolicyShrink
...
M.TabItemFlags_NoAssumedClosure = C.ImGuiTabItemFlags_NoAssumedClosure
--===
...
M.FocusedFlags_NoPopupHierarchy = C.ImGuiFocusedFlags_NoPopupHierarchy
...
M.HoveredFlags_NoPopupHierarchy = C.ImGuiHoveredFlags_NoPopupHierarchy
...
M.HoveredFlags_AllowWhenOverlappedByItem = C.ImGuiHoveredFlags_AllowWhenOverlappedByItem
M.HoveredFlags_AllowWhenOverlappedByWindow = C.ImGuiHoveredFlags_AllowWhenOverlappedByWindow
M.HoveredFlags_AllowWhenDisabled = C.ImGuiHoveredFlags_AllowWhenDisabled
M.HoveredFlags_NoNavOverride = C.ImGuiHoveredFlags_NoNavOverride
...
M.HoveredFlags_ForTooltip = C.ImGuiHoveredFlags_ForTooltip
M.HoveredFlags_Stationary = C.ImGuiHoveredFlags_Stationary
M.HoveredFlags_DelayNone = C.ImGuiHoveredFlags_DelayNone
M.HoveredFlags_DelayShort = C.ImGuiHoveredFlags_DelayShort
M.HoveredFlags_DelayNormal = C.ImGuiHoveredFlags_DelayNormal
M.HoveredFlags_NoSharedDelay = C.ImGuiHoveredFlags_NoSharedDelay
--===
...
M.DragDropFlags_PayloadAutoExpire = C.ImGuiDragDropFlags_PayloadAutoExpire
M.DragDropFlags_PayloadNoCrossContext = C.ImGuiDragDropFlags_PayloadNoCrossContext
M.DragDropFlags_PayloadNoCrossProcess = C.ImGuiDragDropFlags_PayloadNoCrossProcess
...
M.DataType_Bool = C.ImGuiDataType_Bool
M.DataType_String = C.ImGuiDataType_String
...
--===
--=== enum ImGuiKey ===
M.Key_None = C.ImGuiKey_None
M.Key_NamedKey_BEGIN = C.ImGuiKey_NamedKey_BEGIN
...
M.Key_LeftCtrl = C.ImGuiKey_LeftCtrl
M.Key_LeftShift = C.ImGuiKey_LeftShift
M.Key_LeftAlt = C.ImGuiKey_LeftAlt
M.Key_LeftSuper = C.ImGuiKey_LeftSuper
M.Key_RightCtrl = C.ImGuiKey_RightCtrl
M.Key_RightShift = C.ImGuiKey_RightShift
M.Key_RightAlt = C.ImGuiKey_RightAlt
M.Key_RightSuper = C.ImGuiKey_RightSuper
M.Key_Menu = C.ImGuiKey_Menu
M.Key_0 = C.ImGuiKey_0
M.Key_1 = C.ImGuiKey_1
M.Key_2 = C.ImGuiKey_2
M.Key_3 = C.ImGuiKey_3
M.Key_4 = C.ImGuiKey_4
M.Key_5 = C.ImGuiKey_5
M.Key_6 = C.ImGuiKey_6
M.Key_7 = C.ImGuiKey_7
M.Key_8 = C.ImGuiKey_8
M.Key_9 = C.ImGuiKey_9
M.Key_A = C.ImGuiKey_A
M.Key_B = C.ImGuiKey_B
M.Key_C = C.ImGuiKey_C
M.Key_D = C.ImGuiKey_D
M.Key_E = C.ImGuiKey_E
M.Key_F = C.ImGuiKey_F
M.Key_G = C.ImGuiKey_G
M.Key_H = C.ImGuiKey_H
M.Key_I = C.ImGuiKey_I
M.Key_J = C.ImGuiKey_J
M.Key_K = C.ImGuiKey_K
M.Key_L = C.ImGuiKey_L
M.Key_M = C.ImGuiKey_M
M.Key_N = C.ImGuiKey_N
M.Key_O = C.ImGuiKey_O
M.Key_P = C.ImGuiKey_P
M.Key_Q = C.ImGuiKey_Q
M.Key_R = C.ImGuiKey_R
M.Key_S = C.ImGuiKey_S
M.Key_T = C.ImGuiKey_T
M.Key_U = C.ImGuiKey_U
M.Key_V = C.ImGuiKey_V
M.Key_W = C.ImGuiKey_W
M.Key_X = C.ImGuiKey_X
M.Key_Y = C.ImGuiKey_Y
M.Key_Z = C.ImGuiKey_Z
M.Key_F1 = C.ImGuiKey_F1
M.Key_F2 = C.ImGuiKey_F2
M.Key_F3 = C.ImGuiKey_F3
M.Key_F4 = C.ImGuiKey_F4
M.Key_F5 = C.ImGuiKey_F5
M.Key_F6 = C.ImGuiKey_F6
M.Key_F7 = C.ImGuiKey_F7
M.Key_F8 = C.ImGuiKey_F8
M.Key_F9 = C.ImGuiKey_F9
M.Key_F10 = C.ImGuiKey_F10
M.Key_F11 = C.ImGuiKey_F11
M.Key_F12 = C.ImGuiKey_F12
M.Key_F13 = C.ImGuiKey_F13
M.Key_F14 = C.ImGuiKey_F14
M.Key_F15 = C.ImGuiKey_F15
M.Key_F16 = C.ImGuiKey_F16
M.Key_F17 = C.ImGuiKey_F17
M.Key_F18 = C.ImGuiKey_F18
M.Key_F19 = C.ImGuiKey_F19
M.Key_F20 = C.ImGuiKey_F20
M.Key_F21 = C.ImGuiKey_F21
M.Key_F22 = C.ImGuiKey_F22
M.Key_F23 = C.ImGuiKey_F23
M.Key_F24 = C.ImGuiKey_F24
M.Key_Apostrophe = C.ImGuiKey_Apostrophe
M.Key_Comma = C.ImGuiKey_Comma
M.Key_Minus = C.ImGuiKey_Minus
M.Key_Period = C.ImGuiKey_Period
M.Key_Slash = C.ImGuiKey_Slash
M.Key_Semicolon = C.ImGuiKey_Semicolon
M.Key_Equal = C.ImGuiKey_Equal
M.Key_LeftBracket = C.ImGuiKey_LeftBracket
M.Key_Backslash = C.ImGuiKey_Backslash
M.Key_RightBracket = C.ImGuiKey_RightBracket
M.Key_GraveAccent = C.ImGuiKey_GraveAccent
M.Key_CapsLock = C.ImGuiKey_CapsLock
M.Key_ScrollLock = C.ImGuiKey_ScrollLock
M.Key_NumLock = C.ImGuiKey_NumLock
M.Key_PrintScreen = C.ImGuiKey_PrintScreen
M.Key_Pause = C.ImGuiKey_Pause
M.Key_Keypad0 = C.ImGuiKey_Keypad0
M.Key_Keypad1 = C.ImGuiKey_Keypad1
M.Key_Keypad2 = C.ImGuiKey_Keypad2
M.Key_Keypad3 = C.ImGuiKey_Keypad3
M.Key_Keypad4 = C.ImGuiKey_Keypad4
M.Key_Keypad5 = C.ImGuiKey_Keypad5
M.Key_Keypad6 = C.ImGuiKey_Keypad6
M.Key_Keypad7 = C.ImGuiKey_Keypad7
M.Key_Keypad8 = C.ImGuiKey_Keypad8
M.Key_Keypad9 = C.ImGuiKey_Keypad9
M.Key_KeypadDecimal = C.ImGuiKey_KeypadDecimal
M.Key_KeypadDivide = C.ImGuiKey_KeypadDivide
M.Key_KeypadMultiply = C.ImGuiKey_KeypadMultiply
M.Key_KeypadSubtract = C.ImGuiKey_KeypadSubtract
M.Key_KeypadAdd = C.ImGuiKey_KeypadAdd
M.Key_KeypadEnter = C.ImGuiKey_KeypadEnter
M.Key_KeypadEqual = C.ImGuiKey_KeypadEqual
M.Key_AppBack = C.ImGuiKey_AppBack
M.Key_AppForward = C.ImGuiKey_AppForward
M.Key_Oem102 = C.ImGuiKey_Oem102
M.Key_GamepadStart = C.ImGuiKey_GamepadStart
M.Key_GamepadBack = C.ImGuiKey_GamepadBack
M.Key_GamepadFaceLeft = C.ImGuiKey_GamepadFaceLeft
M.Key_GamepadFaceRight = C.ImGuiKey_GamepadFaceRight
M.Key_GamepadFaceUp = C.ImGuiKey_GamepadFaceUp
M.Key_GamepadFaceDown = C.ImGuiKey_GamepadFaceDown
M.Key_GamepadDpadLeft = C.ImGuiKey_GamepadDpadLeft
M.Key_GamepadDpadRight = C.ImGuiKey_GamepadDpadRight
M.Key_GamepadDpadUp = C.ImGuiKey_GamepadDpadUp
M.Key_GamepadDpadDown = C.ImGuiKey_GamepadDpadDown
M.Key_GamepadL1 = C.ImGuiKey_GamepadL1
M.Key_GamepadR1 = C.ImGuiKey_GamepadR1
M.Key_GamepadL2 = C.ImGuiKey_GamepadL2
M.Key_GamepadR2 = C.ImGuiKey_GamepadR2
M.Key_GamepadL3 = C.ImGuiKey_GamepadL3
M.Key_GamepadR3 = C.ImGuiKey_GamepadR3
M.Key_GamepadLStickLeft = C.ImGuiKey_GamepadLStickLeft
M.Key_GamepadLStickRight = C.ImGuiKey_GamepadLStickRight
M.Key_GamepadLStickUp = C.ImGuiKey_GamepadLStickUp
M.Key_GamepadLStickDown = C.ImGuiKey_GamepadLStickDown
M.Key_GamepadRStickLeft = C.ImGuiKey_GamepadRStickLeft
M.Key_GamepadRStickRight = C.ImGuiKey_GamepadRStickRight
M.Key_GamepadRStickUp = C.ImGuiKey_GamepadRStickUp
M.Key_GamepadRStickDown = C.ImGuiKey_GamepadRStickDown
M.Key_MouseLeft = C.ImGuiKey_MouseLeft
M.Key_MouseRight = C.ImGuiKey_MouseRight
M.Key_MouseMiddle = C.ImGuiKey_MouseMiddle
M.Key_MouseX1 = C.ImGuiKey_MouseX1
M.Key_MouseX2 = C.ImGuiKey_MouseX2
M.Key_MouseWheelX = C.ImGuiKey_MouseWheelX
M.Key_MouseWheelY = C.ImGuiKey_MouseWheelY
M.Key_ReservedForModCtrl = C.ImGuiKey_ReservedForModCtrl
M.Key_ReservedForModShift = C.ImGuiKey_ReservedForModShift
M.Key_ReservedForModAlt = C.ImGuiKey_ReservedForModAlt
M.Key_ReservedForModSuper = C.ImGuiKey_ReservedForModSuper
M.Key_NamedKey_END = C.ImGuiKey_NamedKey_END
M.Key_NamedKey_COUNT = C.ImGuiKey_NamedKey_COUNT
M.Mod_None = C.ImGuiMod_None
M.Mod_Ctrl = C.ImGuiMod_Ctrl
M.Mod_Shift = C.ImGuiMod_Shift
M.Mod_Alt = C.ImGuiMod_Alt
M.Mod_Super = C.ImGuiMod_Super
M.Mod_Mask_ = C.ImGuiMod_Mask_
--===
--=== enum ImGuiInputFlags_ ===
M.InputFlags_None = C.ImGuiInputFlags_None
M.InputFlags_Repeat = C.ImGuiInputFlags_Repeat
M.InputFlags_RouteActive = C.ImGuiInputFlags_RouteActive
M.InputFlags_RouteFocused = C.ImGuiInputFlags_RouteFocused
M.InputFlags_RouteGlobal = C.ImGuiInputFlags_RouteGlobal
M.InputFlags_RouteAlways = C.ImGuiInputFlags_RouteAlways
M.InputFlags_RouteOverFocused = C.ImGuiInputFlags_RouteOverFocused
M.InputFlags_RouteOverActive = C.ImGuiInputFlags_RouteOverActive
M.InputFlags_RouteUnlessBgFocused = C.ImGuiInputFlags_RouteUnlessBgFocused
M.InputFlags_RouteFromRootWindow = C.ImGuiInputFlags_RouteFromRootWindow
M.InputFlags_Tooltip = C.ImGuiInputFlags_Tooltip
--===
...
M.ConfigFlags_NoKeyboard = C.ImGuiConfigFlags_NoKeyboard
...
M.BackendFlags_RendererHasTextures = C.ImGuiBackendFlags_RendererHasTextures
--===
--=== enum ImGuiCol_ ===
...
M.Col_InputTextCursor = C.ImGuiCol_InputTextCursor
M.Col_TabHovered = C.ImGuiCol_TabHovered
M.Col_Tab = C.ImGuiCol_Tab
M.Col_TabSelected = C.ImGuiCol_TabSelected
M.Col_TabSelectedOverline = C.ImGuiCol_TabSelectedOverline
M.Col_TabDimmed = C.ImGuiCol_TabDimmed
M.Col_TabDimmedSelected = C.ImGuiCol_TabDimmedSelected
M.Col_TabDimmedSelectedOverline = C.ImGuiCol_TabDimmedSelectedOverline
...
M.Col_TextLink = C.ImGuiCol_TextLink
M.Col_TextSelectedBg = C.ImGuiCol_TextSelectedBg
M.Col_TreeLines = C.ImGuiCol_TreeLines
M.Col_DragDropTarget = C.ImGuiCol_DragDropTarget
M.Col_NavCursor = C.ImGuiCol_NavCursor
...
M.StyleVar_DisabledAlpha = C.ImGuiStyleVar_DisabledAlpha
...
M.StyleVar_ScrollbarPadding = C.ImGuiStyleVar_ScrollbarPadding
M.StyleVar_GrabMinSize = C.ImGuiStyleVar_GrabMinSize
M.StyleVar_GrabRounding = C.ImGuiStyleVar_GrabRounding
M.StyleVar_ImageBorderSize = C.ImGuiStyleVar_ImageBorderSize
M.StyleVar_TabRounding = C.ImGuiStyleVar_TabRounding
M.StyleVar_TabBorderSize = C.ImGuiStyleVar_TabBorderSize
M.StyleVar_TabMinWidthBase = C.ImGuiStyleVar_TabMinWidthBase
M.StyleVar_TabMinWidthShrink = C.ImGuiStyleVar_TabMinWidthShrink
M.StyleVar_TabBarBorderSize = C.ImGuiStyleVar_TabBarBorderSize
M.StyleVar_TabBarOverlineSize = C.ImGuiStyleVar_TabBarOverlineSize
M.StyleVar_TableAngledHeadersAngle = C.ImGuiStyleVar_TableAngledHeadersAngle
M.StyleVar_TableAngledHeadersTextAlign = C.ImGuiStyleVar_TableAngledHeadersTextAlign
M.StyleVar_TreeLinesSize = C.ImGuiStyleVar_TreeLinesSize
M.StyleVar_TreeLinesRounding = C.ImGuiStyleVar_TreeLinesRounding
...
M.StyleVar_SeparatorTextBorderSize = C.ImGuiStyleVar_SeparatorTextBorderSize
M.StyleVar_SeparatorTextAlign = C.ImGuiStyleVar_SeparatorTextAlign
M.StyleVar_SeparatorTextPadding = C.ImGuiStyleVar_SeparatorTextPadding
...
M.ButtonFlags_EnableNav = C.ImGuiButtonFlags_EnableNav
--===
...
M.ColorEditFlags_AlphaOpaque = C.ImGuiColorEditFlags_AlphaOpaque
M.ColorEditFlags_AlphaNoBg = C.ImGuiColorEditFlags_AlphaNoBg
...
M.ColorEditFlags_DefaultOptions_ = C.ImGuiColorEditFlags_DefaultOptions_
M.ColorEditFlags_AlphaMask_ = C.ImGuiColorEditFlags_AlphaMask_
M.ColorEditFlags_DisplayMask_ = C.ImGuiColorEditFlags_DisplayMask_
M.ColorEditFlags_DataTypeMask_ = C.ImGuiColorEditFlags_DataTypeMask_
M.ColorEditFlags_PickerMask_ = C.ImGuiColorEditFlags_PickerMask_
M.ColorEditFlags_InputMask_ = C.ImGuiColorEditFlags_InputMask_
--===
...
M.SliderFlags_WrapAround = C.ImGuiSliderFlags_WrapAround
M.SliderFlags_ClampOnInput = C.ImGuiSliderFlags_ClampOnInput
M.SliderFlags_ClampZeroRange = C.ImGuiSliderFlags_ClampZeroRange
M.SliderFlags_NoSpeedTweaks = C.ImGuiSliderFlags_NoSpeedTweaks
...
M.MouseCursor_Wait = C.ImGuiMouseCursor_Wait
M.MouseCursor_Progress = C.ImGuiMouseCursor_Progress
M.MouseCursor_NotAllowed = C.ImGuiMouseCursor_NotAllowed
M.MouseCursor_COUNT = C.ImGuiMouseCursor_COUNT
--===
--=== enum ImGuiMouseSource ===
M.MouseSource_Mouse = C.ImGuiMouseSource_Mouse
M.MouseSource_TouchScreen = C.ImGuiMouseSource_TouchScreen
M.MouseSource_Pen = C.ImGuiMouseSource_Pen
M.MouseSource_COUNT = C.ImGuiMouseSource_COUNT
--===
--=== enum ImGuiCond_ ===
...
M.TableFlags_HighlightHoveredColumn = C.ImGuiTableFlags_HighlightHoveredColumn
M.TableFlags_SizingMask_ = C.ImGuiTableFlags_SizingMask_
--===
--=== enum ImGuiTableColumnFlags_ ===
M.TableColumnFlags_None = C.ImGuiTableColumnFlags_None
M.TableColumnFlags_Disabled = C.ImGuiTableColumnFlags_Disabled
...
M.TableColumnFlags_NoHeaderLabel = C.ImGuiTableColumnFlags_NoHeaderLabel
...
M.TableColumnFlags_AngledHeader = C.ImGuiTableColumnFlags_AngledHeader
...
--===
--=== struct ImGuiIO ===
function M.ImGuiIO_AddKeyEvent(ImGuiIO_ctx, ImGuiKey_key, bool_down) C.imgui_ImGuiIO_AddKeyEvent(ImGuiIO_ctx, ImGuiKey_key, bool_down) end
function M.ImGuiIO_AddKeyAnalogEvent(ImGuiIO_ctx, ImGuiKey_key, bool_down, float_v) C.imgui_ImGuiIO_AddKeyAnalogEvent(ImGuiIO_ctx, ImGuiKey_key, bool_down, float_v) end
function M.ImGuiIO_AddMousePosEvent(ImGuiIO_ctx, float_x, float_y) C.imgui_ImGuiIO_AddMousePosEvent(ImGuiIO_ctx, float_x, float_y) end
function M.ImGuiIO_AddMouseButtonEvent(ImGuiIO_ctx, int_button, bool_down) C.imgui_ImGuiIO_AddMouseButtonEvent(ImGuiIO_ctx, int_button, bool_down) end
function M.ImGuiIO_AddMouseWheelEvent(ImGuiIO_ctx, float_wheel_x, float_wheel_y) C.imgui_ImGuiIO_AddMouseWheelEvent(ImGuiIO_ctx, float_wheel_x, float_wheel_y) end
function M.ImGuiIO_AddMouseSourceEvent(ImGuiIO_ctx, ImGuiMouseSource_source) C.imgui_ImGuiIO_AddMouseSourceEvent(ImGuiIO_ctx, ImGuiMouseSource_source) end
...
function M.ImGuiIO_SetAppAcceptingEvents(ImGuiIO_ctx, bool_accepting_events) C.imgui_ImGuiIO_SetAppAcceptingEvents(ImGuiIO_ctx, bool_accepting_events) end
...
function M.ImGuiIO_ClearInputMouse(ImGuiIO_ctx) C.imgui_ImGuiIO_ClearInputMouse(ImGuiIO_ctx) end
...
function M.ImGuiTextFilter_Draw(ImGuiTextFilter_ctx, string_label, float_width) 
  ...
  if float_width == nil then float_width = 0.0 end
  if string_label == nil then log("E", "", "Parameter 'string_label' of function 'Draw' cannot be nil, as the c type is 'const char *'") ; return end
...
function M.ImGuiTextBuffer_resize(ImGuiTextBuffer_ctx, int_size) C.imgui_ImGuiTextBuffer_resize(ImGuiTextBuffer_ctx, int_size) end
...
function M.ImGuiStoragePair(key, val)
  local res = ffi.new("ImGuiStoragePair")
  res.key = key
  res.val = val
  return res
end
function M.ImGuiStoragePairPtr(key, val)end
function M.ImGuiStoragePair(key, val)
  local res = ffi.new("ImGuiStoragePair")
  res.key = key
  res.val = val
...
function M.ImGuiStorage_GetFloat(ImGuiStorage_ctx, ImGuiID_key, float_default_val) 
  if float_default_val == nil then float_default_val = 0.0 end
...
M.ListClipperFlags_None = C.ImGuiListClipperFlags_None
M.ListClipperFlags_NoSetTableRowCounters = C.ImGuiListClipperFlags_NoSetTableRowCounters
--===
...
function M.ImGuiListClipper_Begin(ImGuiListClipper_ctx, int_items_count, float_items_height) 
  if float_items_height == nil then float_items_height = - 1.0 end
...
function M.ImGuiListClipper_IncludeItemByIndex(ImGuiListClipper_ctx, int_item_index) C.imgui_ImGuiListClipper_IncludeItemByIndex(ImGuiListClipper_ctx, int_item_index) end
function M.ImGuiListClipper_IncludeItemsByIndex(ImGuiListClipper_ctx, int_item_begin, int_item_end) C.imgui_ImGuiListClipper_IncludeItemsByIndex(ImGuiListClipper_ctx, int_item_begin, int_item_end) end
function M.ImGuiListClipper_SeekCursorForItem(ImGuiListClipper_ctx, int_item_index) C.imgui_ImGuiListClipper_SeekCursorForItem(ImGuiListClipper_ctx, int_item_index) end
--===
--=== struct ImColor ===
...
function M.ImColor_SetHSV(ImColor_ctx, float_h, float_s, float_v, float_a) 
  if float_a == nil then float_a = 1.0 end
...
M.MultiSelectFlags_None = C.ImGuiMultiSelectFlags_None
M.MultiSelectFlags_SingleSelect = C.ImGuiMultiSelectFlags_SingleSelect
M.MultiSelectFlags_NoSelectAll = C.ImGuiMultiSelectFlags_NoSelectAll
M.MultiSelectFlags_NoRangeSelect = C.ImGuiMultiSelectFlags_NoRangeSelect
M.MultiSelectFlags_NoAutoSelect = C.ImGuiMultiSelectFlags_NoAutoSelect
M.MultiSelectFlags_NoAutoClear = C.ImGuiMultiSelectFlags_NoAutoClear
M.MultiSelectFlags_NoAutoClearOnReselect = C.ImGuiMultiSelectFlags_NoAutoClearOnReselect
M.MultiSelectFlags_BoxSelect1d = C.ImGuiMultiSelectFlags_BoxSelect1d
M.MultiSelectFlags_BoxSelect2d = C.ImGuiMultiSelectFlags_BoxSelect2d
M.MultiSelectFlags_BoxSelectNoScroll = C.ImGuiMultiSelectFlags_BoxSelectNoScroll
M.MultiSelectFlags_ClearOnEscape = C.ImGuiMultiSelectFlags_ClearOnEscape
M.MultiSelectFlags_ClearOnClickVoid = C.ImGuiMultiSelectFlags_ClearOnClickVoid
M.MultiSelectFlags_ScopeWindow = C.ImGuiMultiSelectFlags_ScopeWindow
M.MultiSelectFlags_ScopeRect = C.ImGuiMultiSelectFlags_ScopeRect
M.MultiSelectFlags_SelectOnClick = C.ImGuiMultiSelectFlags_SelectOnClick
M.MultiSelectFlags_SelectOnClickRelease = C.ImGuiMultiSelectFlags_SelectOnClickRelease
M.MultiSelectFlags_NavWrapX = C.ImGuiMultiSelectFlags_NavWrapX
--===
--=== struct ImGuiMultiSelectIO ===
--===
--=== enum ImGuiSelectionRequestType ===
M.SelectionRequestType_None = C.ImGuiSelectionRequestType_None
M.SelectionRequestType_SetAll = C.ImGuiSelectionRequestType_SetAll
M.SelectionRequestType_SetRange = C.ImGuiSelectionRequestType_SetRange
--===
--=== struct ImGuiSelectionRequest ===
--===
--=== struct ImGuiSelectionBasicStorage ===
function M.ImGuiSelectionBasicStorage() return ffi.new("ImGuiSelectionBasicStorage") end
function M.ImGuiSelectionBasicStoragePtr() return ffi.new("ImGuiSelectionBasicStorage[1]") end
function M.ImGuiSelectionBasicStorage_ApplyRequests(ImGuiSelectionBasicStorage_ctx, ImGuiMultiSelectIO_ms_io) 
  if ImGuiMultiSelectIO_ms_io == nil then log("E", "", "Parameter 'ImGuiMultiSelectIO_ms_io' of function 'ApplyRequests' cannot be nil, as the c type is 'ImGuiMultiSelectIO *'") ; return end
  C.imgui_ImGuiSelectionBasicStorage_ApplyRequests(ImGuiSelectionBasicStorage_ctx, ImGuiMultiSelectIO_ms_io)
end
function M.ImGuiSelectionBasicStorage_Contains(ImGuiSelectionBasicStorage_ctx, ImGuiID_id) return C.imgui_ImGuiSelectionBasicStorage_Contains(ImGuiSelectionBasicStorage_ctx, ImGuiID_id) end
function M.ImGuiSelectionBasicStorage_Clear(ImGuiSelectionBasicStorage_ctx) C.imgui_ImGuiSelectionBasicStorage_Clear(ImGuiSelectionBasicStorage_ctx) end
function M.ImGuiSelectionBasicStorage_Swap(ImGuiSelectionBasicStorage_ctx, ImGuiSelectionBasicStorage_r) C.imgui_ImGuiSelectionBasicStorage_Swap(ImGuiSelectionBasicStorage_ctx, ImGuiSelectionBasicStorage_r) end
function M.ImGuiSelectionBasicStorage_SetItemSelected(ImGuiSelectionBasicStorage_ctx, ImGuiID_id, bool_selected) C.imgui_ImGuiSelectionBasicStorage_SetItemSelected(ImGuiSelectionBasicStorage_ctx, ImGuiID_id, bool_selected) end
function M.ImGuiSelectionBasicStorage_GetNextSelectedItem(ImGuiSelectionBasicStorage_ctx, void_opaque_it, ImGuiID_out_id) 
  if void_opaque_it == nil then log("E", "", "Parameter 'void_opaque_it' of function 'GetNextSelectedItem' cannot be nil, as the c type is 'void **'") ; return end
  if ImGuiID_out_id == nil then log("E", "", "Parameter 'ImGuiID_out_id' of function 'GetNextSelectedItem' cannot be nil, as the c type is 'ImGuiID *'") ; return end
  return C.imgui_ImGuiSelectionBasicStorage_GetNextSelectedItem(ImGuiSelectionBasicStorage_ctx, void_opaque_it, ImGuiID_out_id)
end
function M.ImGuiSelectionBasicStorage_GetStorageIdFromIndex(ImGuiSelectionBasicStorage_ctx, int_idx) return C.imgui_ImGuiSelectionBasicStorage_GetStorageIdFromIndex(ImGuiSelectionBasicStorage_ctx, int_idx) end
--===
--=== struct ImGuiSelectionExternalStorage ===
function M.ImGuiSelectionExternalStorage() return ffi.new("ImGuiSelectionExternalStorage") end
function M.ImGuiSelectionExternalStoragePtr() return ffi.new("ImGuiSelectionExternalStorage[1]") end
function M.ImGuiSelectionExternalStorage_ApplyRequests(ImGuiSelectionExternalStorage_ctx, ImGuiMultiSelectIO_ms_io) 
  if ImGuiMultiSelectIO_ms_io == nil then log("E", "", "Parameter 'ImGuiMultiSelectIO_ms_io' of function 'ApplyRequests' cannot be nil, as the c type is 'ImGuiMultiSelectIO *'") ; return end
  C.imgui_ImGuiSelectionExternalStorage_ApplyRequests(ImGuiSelectionExternalStorage_ctx, ImGuiMultiSelectIO_ms_io)
end
--===
--=== struct ImDrawCmd ===
...
--===
--=== enum ImDrawFlags_ ===
M.ImDrawFlags_None = C.ImDrawFlags_None
M.ImDrawFlags_Closed = C.ImDrawFlags_Closed
M.ImDrawFlags_RoundCornersTopLeft = C.ImDrawFlags_RoundCornersTopLeft
M.ImDrawFlags_RoundCornersTopRight = C.ImDrawFlags_RoundCornersTopRight
M.ImDrawFlags_RoundCornersBottomLeft = C.ImDrawFlags_RoundCornersBottomLeft
M.ImDrawFlags_RoundCornersBottomRight = C.ImDrawFlags_RoundCornersBottomRight
M.ImDrawFlags_RoundCornersNone = C.ImDrawFlags_RoundCornersNone
M.ImDrawFlags_RoundCornersTop = C.ImDrawFlags_RoundCornersTop
M.ImDrawFlags_RoundCornersBottom = C.ImDrawFlags_RoundCornersBottom
M.ImDrawFlags_RoundCornersLeft = C.ImDrawFlags_RoundCornersLeft
M.ImDrawFlags_RoundCornersRight = C.ImDrawFlags_RoundCornersRight
M.ImDrawFlags_RoundCornersAll = C.ImDrawFlags_RoundCornersAll
M.ImDrawFlags_RoundCornersDefault_ = C.ImDrawFlags_RoundCornersDefault_
M.ImDrawFlags_RoundCornersMask_ = C.ImDrawFlags_RoundCornersMask_
--===
...
function M.ImDrawList_PushTexture(ImDrawList_ctx, ImTextureID_tex_ref) C.imgui_ImDrawList_PushTexture(ImDrawList_ctx, ImTextureID_tex_ref) end
function M.ImDrawList_PopTexture(ImDrawList_ctx) C.imgui_ImDrawList_PopTexture(ImDrawList_ctx) end
...
function M.ImDrawList_AddLine(ImDrawList_ctx, ImVec2_p1, ImVec2_p2, ImU32_col, float_thickness) 
  if float_thickness == nil then float_thickness = 1.0 end
...
function M.ImDrawList_AddRect(ImDrawList_ctx, ImVec2_p_min, ImVec2_p_max, ImU32_col, float_rounding, ImDrawFlags_flags, float_thickness) 
  if float_rounding == nil then float_rounding = 0.0 end
...
function M.ImDrawList_AddEllipse(ImDrawList_ctx, ImVec2_center, ImVec2_radius, ImU32_col, float_rot, int_num_segments, float_thickness) 
  if float_rot == nil then float_rot = 0.0 end
  if int_num_segments == nil then int_num_segments = 0 end
  if float_thickness == nil then float_thickness = 1.0 end
  C.imgui_ImDrawList_AddEllipse(ImDrawList_ctx, ImVec2_center, ImVec2_radius, ImU32_col, float_rot, int_num_segments, float_thickness)
end
function M.ImDrawList_AddEllipseFilled(ImDrawList_ctx, ImVec2_center, ImVec2_radius, ImU32_col, float_rot, int_num_segments) 
  if float_rot == nil then float_rot = 0.0 end
  if int_num_segments == nil then int_num_segments = 0 end
  C.imgui_ImDrawList_AddEllipseFilled(ImDrawList_ctx, ImVec2_center, ImVec2_radius, ImU32_col, float_rot, int_num_segments)
end
...
function M.ImDrawList_AddText2(ImDrawList_ctx, ImFont_font, float_font_size, ImVec2_pos, ImU32_col, string_text_begin, string_text_end, float_wrap_width, ImVec4_cpu_fine_clip_rect) 
  -- string_text_end is optional and can be nil
  if float_wrap_width == nil then float_wrap_width = 0.0 end
  -- ImVec4_cpu_fine_clip_rect is optional and can be nil
  if ImFont_font == nil then log("E", "", "Parameter 'ImFont_font' of function 'AddText2' cannot be nil, as the c type is 'ImFont *'") ; return end
...
function M.ImDrawList_AddConcavePolyFilled(ImDrawList_ctx, ImVec2_points, int_num_points, ImU32_col) 
  if ImVec2_points == nil then log("E", "", "Parameter 'ImVec2_points' of function 'AddConcavePolyFilled' cannot be nil, as the c type is 'const ImVec2 *'") ; return end
  C.imgui_ImDrawList_AddConcavePolyFilled(ImDrawList_ctx, ImVec2_points, int_num_points, ImU32_col)
end
function M.ImDrawList_AddImage(ImDrawList_ctx, ImTextureID_tex_ref, ImVec2_p_min, ImVec2_p_max, ImVec2_uv_min, ImVec2_uv_max, ImU32_col) 
  if ImVec2_uv_min == nil then ImVec2_uv_min = ffi.new("ImVec2_C",  0, 0) end
  if ImVec2_uv_max == nil then ImVec2_uv_max = ffi.new("ImVec2_C",  1, 1) end
  if ImU32_col == nil then ImU32_col = IM_COL32_WHITE end
  C.imgui_ImDrawList_AddImage(ImDrawList_ctx, ImTextureID_tex_ref, ImVec2_p_min, ImVec2_p_max, ImVec2_uv_min, ImVec2_uv_max, ImU32_col)
end
function M.ImDrawList_AddImageQuad(ImDrawList_ctx, ImTextureID_tex_ref, ImVec2_p1, ImVec2_p2, ImVec2_p3, ImVec2_p4, ImVec2_uv1, ImVec2_uv2, ImVec2_uv3, ImVec2_uv4, ImU32_col) 
  if ImVec2_uv1 == nil then ImVec2_uv1 = ffi.new("ImVec2_C",  0, 0) end
  if ImVec2_uv2 == nil then ImVec2_uv2 = ffi.new("ImVec2_C",  1, 0) end
  if ImVec2_uv3 == nil then ImVec2_uv3 = ffi.new("ImVec2_C",  1, 1) end
  if ImVec2_uv4 == nil then ImVec2_uv4 = ffi.new("ImVec2_C",  0, 1) end
  if ImU32_col == nil then ImU32_col = IM_COL32_WHITE end
  C.imgui_ImDrawList_AddImageQuad(ImDrawList_ctx, ImTextureID_tex_ref, ImVec2_p1, ImVec2_p2, ImVec2_p3, ImVec2_p4, ImVec2_uv1, ImVec2_uv2, ImVec2_uv3, ImVec2_uv4, ImU32_col)
end
function M.ImDrawList_AddImageRounded(ImDrawList_ctx, ImTextureID_tex_ref, ImVec2_p_min, ImVec2_p_max, ImVec2_uv_min, ImVec2_uv_max, ImU32_col, float_rounding, ImDrawFlags_flags) 
  if ImDrawFlags_flags == nil then ImDrawFlags_flags = 0 end
  C.imgui_ImDrawList_AddImageRounded(ImDrawList_ctx, ImTextureID_tex_ref, ImVec2_p_min, ImVec2_p_max, ImVec2_uv_min, ImVec2_uv_max, ImU32_col, float_rounding, ImDrawFlags_flags)
end
...
function M.ImDrawList_PathFillConcave(ImDrawList_ctx, ImU32_col) C.imgui_ImDrawList_PathFillConcave(ImDrawList_ctx, ImU32_col) end
...
function M.ImDrawList_PathEllipticalArcTo(ImDrawList_ctx, ImVec2_center, ImVec2_radius, float_rot, float_a_min, float_a_max, int_num_segments) 
  if int_num_segments == nil then int_num_segments = 0 end
  C.imgui_ImDrawList_PathEllipticalArcTo(ImDrawList_ctx, ImVec2_center, ImVec2_radius, float_rot, float_a_min, float_a_max, int_num_segments)
end
...
function M.ImDrawList_AddCallback(ImDrawList_ctx, ImDrawCallback_callback, void_userdata, size_t_userdata_size) 
  if size_t_userdata_size == nil then size_t_userdata_size = 0 end
  if void_userdata == nil then log("E", "", "Parameter 'void_userdata' of function 'AddCallback' cannot be nil, as the c type is 'void *'") ; return end
  C.imgui_ImDrawList_AddCallback(ImDrawList_ctx, ImDrawCallback_callback, void_userdata, size_t_userdata_size)
end
...
function M.ImDrawList__SetDrawListSharedData(ImDrawList_ctx, ImDrawListSharedData_data) 
  if ImDrawListSharedData_data == nil then log("E", "", "Parameter 'ImDrawListSharedData_data' of function '_SetDrawListSharedData' cannot be nil, as the c type is 'ImDrawListSharedData *'") ; return end
  C.imgui_ImDrawList__SetDrawListSharedData(ImDrawList_ctx, ImDrawListSharedData_data)
end
...
function M.ImDrawList__TryMergeDrawCmds(ImDrawList_ctx) C.imgui_ImDrawList__TryMergeDrawCmds(ImDrawList_ctx) end
function M.ImDrawList__OnChangedClipRect(ImDrawList_ctx) C.imgui_ImDrawList__OnChangedClipRect(ImDrawList_ctx) end
function M.ImDrawList__OnChangedTexture(ImDrawList_ctx) C.imgui_ImDrawList__OnChangedTexture(ImDrawList_ctx) end
function M.ImDrawList__OnChangedVtxOffset(ImDrawList_ctx) C.imgui_ImDrawList__OnChangedVtxOffset(ImDrawList_ctx) end
function M.ImDrawList__SetTexture(ImDrawList_ctx, ImTextureID_tex_ref) C.imgui_ImDrawList__SetTexture(ImDrawList_ctx, ImTextureID_tex_ref) end
function M.ImDrawList__CalcCircleAutoSegmentCount(ImDrawList_ctx, float_radius) return C.imgui_ImDrawList__CalcCircleAutoSegmentCount(ImDrawList_ctx, float_radius) end
function M.ImDrawList__PathArcToFastEx(ImDrawList_ctx, ImVec2_center, float_radius, int_a_min_sample, int_a_max_sample, int_a_step) C.imgui_ImDrawList__PathArcToFastEx(ImDrawList_ctx, ImVec2_center, float_radius, int_a_min_sample, int_a_max_sample, int_a_step) end
function M.ImDrawList__PathArcToN(ImDrawList_ctx, ImVec2_center, float_radius, float_a_min, float_a_max, int_num_segments) C.imgui_ImDrawList__PathArcToN(ImDrawList_ctx, ImVec2_center, float_radius, float_a_min, float_a_max, int_num_segments) end
--===
--=== struct ImDrawData ===
...
M.ImTextureFormat_RGBA32 = C.ImTextureFormat_RGBA32
M.ImTextureFormat_Alpha8 = C.ImTextureFormat_Alpha8
--===
--=== enum ImTextureStatus ===
M.ImTextureStatus_OK = C.ImTextureStatus_OK
M.ImTextureStatus_Destroyed = C.ImTextureStatus_Destroyed
M.ImTextureStatus_WantCreate = C.ImTextureStatus_WantCreate
M.ImTextureStatus_WantUpdates = C.ImTextureStatus_WantUpdates
M.ImTextureStatus_WantDestroy = C.ImTextureStatus_WantDestroy
--===
--=== struct ImTextureRect ===
--===
--=== struct ImTextureData ===
function M.ImTextureData() return ffi.new("ImTextureData") end
function M.ImTextureDataPtr() return ffi.new("ImTextureData[1]") end
function M.ImTextureData_Create(ImTextureData_ctx, ImTextureFormat_format, int_w, int_h) C.imgui_ImTextureData_Create(ImTextureData_ctx, ImTextureFormat_format, int_w, int_h) end
function M.ImTextureData_DestroyPixels(ImTextureData_ctx) C.imgui_ImTextureData_DestroyPixels(ImTextureData_ctx) end
function M.ImTextureData_GetPixels(ImTextureData_ctx) return C.imgui_ImTextureData_GetPixels(ImTextureData_ctx) end
function M.ImTextureData_GetPixelsAt(ImTextureData_ctx, int_x, int_y) return C.imgui_ImTextureData_GetPixelsAt(ImTextureData_ctx, int_x, int_y) end
function M.ImTextureData_GetSizeInBytes(ImTextureData_ctx) return C.imgui_ImTextureData_GetSizeInBytes(ImTextureData_ctx) end
function M.ImTextureData_GetPitch(ImTextureData_ctx) return C.imgui_ImTextureData_GetPitch(ImTextureData_ctx) end
function M.ImTextureData_GetTexRef(ImTextureData_ctx) return C.imgui_ImTextureData_GetTexRef(ImTextureData_ctx) end
function M.ImTextureData_SetTexID(ImTextureData_ctx, ImTextureID_tex_id) C.imgui_ImTextureData_SetTexID(ImTextureData_ctx, ImTextureID_tex_id) end
function M.ImTextureData_SetStatus(ImTextureData_ctx, ImTextureStatus_status) C.imgui_ImTextureData_SetStatus(ImTextureData_ctx, ImTextureStatus_status) end
--===
--=== struct ImFontConfig ===
...
--===
--=== struct ImFontGlyph ===
function M.ImFontGlyph() return ffi.new("ImFontGlyph") end
function M.ImFontGlyphPtr() return ffi.new("ImFontGlyph[1]") end
--===
...
function M.ImFontAtlasRect() return ffi.new("ImFontAtlasRect") end
function M.ImFontAtlasRectPtr() return ffi.new("ImFontAtlasRect[1]") end
--===
...
function M.ImFontAtlas_AddFontFromFileTTF(ImFontAtlas_ctx, string_filename, float_size_pixels, ImFontConfig_font_cfg, ImWchar_glyph_ranges) 
  if float_size_pixels == nil then float_size_pixels = 0.0 end
  -- ImFontConfig_font_cfg is optional and can be nil
...
function M.ImFontAtlas_AddFontFromMemoryTTF(ImFontAtlas_ctx, void_font_data, int_font_data_size, float_size_pixels, ImFontConfig_font_cfg, ImWchar_glyph_ranges) 
  if float_size_pixels == nil then float_size_pixels = 0.0 end
  -- ImFontConfig_font_cfg is optional and can be nil
  -- ImWchar_glyph_ranges is optional and can be nil
  ...
  return C.imgui_ImFontAtlas_AddFontFromMemoryTTF(ImFontAtlas_ctx, void_font_data, int_font_data_size, float_size_pixels, ImFontConfig_font_cfg, ImWchar_glyph_ranges)
end
function M.ImFontAtlas_AddFontFromMemoryCompressedTTF(ImFontAtlas_ctx, void_compressed_font_data, int_compressed_font_data_size, float_size_pixels, ImFontConfig_font_cfg, ImWchar_glyph_ranges) 
  if float_size_pixels == nil then float_size_pixels = 0.0 end
  -- ImFontConfig_font_cfg is optional and can be nil
  -- ImWchar_glyph_ranges is optional and can be nil
  ...
  return C.imgui_ImFontAtlas_AddFontFromMemoryCompressedTTF(ImFontAtlas_ctx, void_compressed_font_data, int_compressed_font_data_size, float_size_pixels, ImFontConfig_font_cfg, ImWchar_glyph_ranges)
end
...
function M.ImFontAtlas_RemoveFont(ImFontAtlas_ctx, ImFont_font) 
  if ImFont_font == nil then log("E", "", "Parameter 'ImFont_font' of function 'RemoveFont' cannot be nil, as the c type is 'ImFont *'") ; return end
  C.imgui_ImFontAtlas_RemoveFont(ImFontAtlas_ctx, ImFont_font)
end
function M.ImFontAtlas_Clear(ImFontAtlas_ctx) C.imgui_ImFontAtlas_Clear(ImFontAtlas_ctx) end
function M.ImFontAtlas_CompactCache(ImFontAtlas_ctx) C.imgui_ImFontAtlas_CompactCache(ImFontAtlas_ctx) end
function M.ImFontAtlas_SetFontLoader(ImFontAtlas_ctx, ImFontLoader_font_loader) 
  if ImFontLoader_font_loader == nil then log("E", "", "Parameter 'ImFontLoader_font_loader' of function 'SetFontLoader' cannot be nil, as the c type is 'const ImFontLoader *'") ; return end
  C.imgui_ImFontAtlas_SetFontLoader(ImFontAtlas_ctx, ImFontLoader_font_loader)
end
...
function M.ImFontAtlas_AddCustomRect(ImFontAtlas_ctx, int_width, int_height, ImFontAtlasRect_out_r) 
  -- ImFontAtlasRect_out_r is optional and can be nil
  return C.imgui_ImFontAtlas_AddCustomRect(ImFontAtlas_ctx, int_width, int_height, ImFontAtlasRect_out_r)
end
function M.ImFontAtlas_RemoveCustomRect(ImFontAtlas_ctx, ImFontAtlasRectId_id) C.imgui_ImFontAtlas_RemoveCustomRect(ImFontAtlas_ctx, ImFontAtlasRectId_id) end
function M.ImFontAtlas_GetCustomRect(ImFontAtlas_ctx, ImFontAtlasRectId_id, ImFontAtlasRect_out_r) 
  if ImFontAtlasRect_out_r == nil then log("E", "", "Parameter 'ImFontAtlasRect_out_r' of function 'GetCustomRect' cannot be nil, as the c type is 'ImFontAtlasRect *'") ; return end
  return C.imgui_ImFontAtlas_GetCustomRect(ImFontAtlas_ctx, ImFontAtlasRectId_id, ImFontAtlasRect_out_r)
end
--===
--=== struct ImFontBaked ===
function M.ImFontBaked() return ffi.new("ImFontBaked") end
function M.ImFontBakedPtr() return ffi.new("ImFontBaked[1]") end
function M.ImFontBaked_ClearOutputData(ImFontBaked_ctx) C.imgui_ImFontBaked_ClearOutputData(ImFontBaked_ctx) end
function M.ImFontBaked_FindGlyph(ImFontBaked_ctx, ImWchar_c) return C.imgui_ImFontBaked_FindGlyph(ImFontBaked_ctx, ImWchar_c) end
function M.ImFontBaked_FindGlyphNoFallback(ImFontBaked_ctx, ImWchar_c) return C.imgui_ImFontBaked_FindGlyphNoFallback(ImFontBaked_ctx, ImWchar_c) end
function M.ImFontBaked_GetCharAdvance(ImFontBaked_ctx, ImWchar_c) return C.imgui_ImFontBaked_GetCharAdvance(ImFontBaked_ctx, ImWchar_c) end
function M.ImFontBaked_IsGlyphLoaded(ImFontBaked_ctx, ImWchar_c) return C.imgui_ImFontBaked_IsGlyphLoaded(ImFontBaked_ctx, ImWchar_c) end
--===
--=== enum ImFontFlags_ ===
M.ImFontFlags_None = C.ImFontFlags_None
M.ImFontFlags_NoLoadError = C.ImFontFlags_NoLoadError
M.ImFontFlags_NoLoadGlyphs = C.ImFontFlags_NoLoadGlyphs
M.ImFontFlags_LockBakedSizes = C.ImFontFlags_LockBakedSizes
--===
--=== struct ImFont ===
function M.ImFont() return ffi.new("ImFont") end
function M.ImFontPtr() return ffi.new("ImFont[1]") end
function M.ImFont_IsGlyphInFont(ImFont_ctx, ImWchar_c) return C.imgui_ImFont_IsGlyphInFont(ImFont_ctx, ImWchar_c) end
...
function M.ImFont_GetFontBaked(ImFont_ctx, float_font_size, float_density) 
  if float_density == nil then float_density = - 1.0 end
  return C.imgui_ImFont_GetFontBaked(ImFont_ctx, float_font_size, float_density)
end
function M.ImFont_CalcTextSizeA(ImFont_ctx, float_size, float_max_width, float_wrap_width, string_text_begin, string_text_end, string_out_remaining) 
  -- string_text_end is optional and can be nil
  -- string_out_remaining is optional and can be nil
  ...
  return C.imgui_ImFont_CalcTextSizeA(ImFont_ctx, float_size, float_max_width, float_wrap_width, string_text_begin, string_text_end, string_out_remaining)
end
function M.ImFont_CalcWordWrapPosition(ImFont_ctx, float_size, string_text, string_text_end, float_wrap_width) 
  if string_text == nil then log("E", "", "Parameter 'string_text' of function 'CalcWordWrapPosition' cannot be nil, as the c type is 'const char *'") ; return end
  if string_text_end == nil then log("E", "", "Parameter 'string_text_end' of function 'CalcWordWrapPosition' cannot be nil, as the c type is 'const char *'") ; return end
  return C.imgui_ImFont_CalcWordWrapPosition(ImFont_ctx, float_size, string_text, string_text_end, float_wrap_width)
end
function M.ImFont_RenderChar(ImFont_ctx, ImDrawList_draw_list, float_size, ImVec2_pos, ImU32_col, ImWchar_c, ImVec4_cpu_fine_clip) 
  -- ImVec4_cpu_fine_clip is optional and can be nil
  ...
  C.imgui_ImFont_RenderChar(ImFont_ctx, ImDrawList_draw_list, float_size, ImVec2_pos, ImU32_col, ImWchar_c, ImVec4_cpu_fine_clip)
end
function M.ImFont_RenderText(ImFont_ctx, ImDrawList_draw_list, float_size, ImVec2_pos, ImU32_col, ImVec4_clip_rect, string_text_begin, string_text_end, float_wrap_width, ImDrawTextFlags_flags) 
  if float_wrap_width == nil then float_wrap_width = 0.0 end
  if ImDrawTextFlags_flags == nil then ImDrawTextFlags_flags = 0 end
  ...
  C.imgui_ImFont_RenderText(ImFont_ctx, ImDrawList_draw_list, float_size, ImVec2_pos, ImU32_col, ImVec4_clip_rect, string_text_begin, string_text_end, float_wrap_width, ImDrawTextFlags_flags)
end
function M.ImFont_ClearOutputData(ImFont_ctx) C.imgui_ImFont_ClearOutputData(ImFont_ctx) end
function M.ImFont_AddRemapChar(ImFont_ctx, ImWchar_from_codepoint, ImWchar_to_codepoint) C.imgui_ImFont_AddRemapChar(ImFont_ctx, ImWchar_from_codepoint, ImWchar_to_codepoint) end
...
M.ViewportFlags_IsPlatformWindow = C.ImGuiViewportFlags_IsPlatformWindow
M.ViewportFlags_IsPlatformMonitor = C.ImGuiViewportFlags_IsPlatformMonitor
M.ViewportFlags_OwnedByApp = C.ImGuiViewportFlags_OwnedByApp
--===
...
function M.ImGuiViewport_GetWorkCenter(ImGuiViewport_ctx) return C.imgui_ImGuiViewport_GetWorkCenter(ImGuiViewport_ctx) end
--===
...
function M.ImGuiPlatformImeData() return ffi.new("ImGuiPlatformImeData") end
function M.ImGuiPlatformImeDataPtr() return ffi.new("ImGuiPlatformImeData[1]") end
--===
